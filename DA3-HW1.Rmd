...
#13#
atasummary(w*factor(educ)*factor(white)  ~ N + Percent() + Mean, data = dt )#3
...
model3 <- as.formula(w ~ educ+age+agesq+region+ownchild+factor(white)*gender+married_status*gender+unionmme*Sector+unionmme*gender)

model4 <- as.formula(w ~ educ+age+agesq+region+ownchild+factor(white)*gender+married_status*gender+unionmme*Sector+unionmme*gender+factor(educ)*region+factor(white)*factor(married_status)+Sector*region+unionmme*gender+Sector*gender)
...
Before we could start running regression model we had to account for the interaction between the variable and decide which predictor to choose. The base for selection of predictor was if the wage differential was greater than $2 and the number of observation were sufficient enough to make a comparison. Since we are looking at hourly wage even 2 dollar difference would make a huge difference if we see the weekly or the monthly scale.
