...
# data %>% group_by(property_type,n_accommodates) %>% summarise(count=n())
# pp <- data %>% group_by(f_neighbourhood_cleansed) %>% summarise(mean=mean(price)) %>% 
#   arrange(-mean) %>% head(5)
# 
# ppp <- data %>% filter(property_type=="loft") %>% group_by(f_neighbourhood_cleansed)%>% 
#   summarise(count=n())
# 
# pppp <- data %>% filter(property_type=="loft" & f_neighbourhood_cleansed == "DUOMO " ) %>% group_by(n_accommodates,f_neighbourhood_cleansed)%>% 
#   summarise(count=n())
