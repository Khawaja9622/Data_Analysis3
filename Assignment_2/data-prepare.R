...
# neighbourhood_cleansed (58 missing)
df %>% 
...
df$f_neighbourhood_cleansed <- factor(df$f_neighbourhood_cleansed)
 ...
 df$neighbourhood_cleansed <- NULL
 ...
 # Pool accommodations with 1,2,6 bathrooms
 df <-  df %>%
   mutate(f_bathrooms = cut(n_bathrooms, c(1,2,6), labels=c(1,2), right = F) )
unique(df$f_bathrooms)
...
#Pool accommodations with 1,2,3,9 bedroom
df <- df %>%
...
############################
## _n_number_of_reviews
skimr::skim(df$n_number_of_reviews)

ggplot(data = df, aes(x=log(n_number_of_reviews ), y=price)) +
...
categoricals <- c("f_property_type")
...
df <- df %>% rename(d_family_friendly = d_have_childrenbabycribhighcornerchang,
                    ...
                    d_shampoo_toiletries = d_have_toiletriesshampooconditionerbody_soapgel,
...
write_csv(df, paste0(data_out, "airbnb_Milan_workfile_adj.csv"))
saveRDS(df, paste0(data_out, "airbnb_Milan_workfile_adj.rds"))
